...
history is rendered with the served model's chat template, sent to the sampling
backend as token_ids, and fed into a shared TrajectoryManager keyed by session id.
finish_session(sid) drains a session's trajectory into a list of TraceRecord.
