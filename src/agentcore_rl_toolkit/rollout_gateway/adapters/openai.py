...
Speaks the OpenAI /v1/chat/completions protocol so an OpenAI-compatible client can
drive the gateway. Each request is rendered with the served model's chat template,
sent to the sampling backend as token_ids, parsed, and folded into a shared
TrajectoryManager keyed by session id. finish_session(sid) drains a session's
trajectory into a list of TraceRecord.

Only /v1/chat/completions is implemented; the Responses API (/v1/responses) is not
yet supported.
"""
...
from ..render import ParsedOutput
from .common import (
    BaseAdapter,
    Reply,
    flatten_content,
...
def _build_reply_parts(parsed: ParsedOutput, finish: str) -> tuple[dict[str, Any], dict[str, Any], str]:
...
def _render_response(
    ...
    return {
        ...
        "model": body.get("model", "acr-gateway"),
        "choices": [
            {
                "index": 0,
...
async def _render_stream(
    ...
    (we have no token-level deltas from the backend here), so we emit one role
